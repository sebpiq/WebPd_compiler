...
type Frame = {[portletId: string]: Array<PdSharedTypes.ControlValue> | PdSharedTypes.SignalValue}
...
export const generateFramesForNode = async (
    ...
    const testNodeInlets = DEFAULT_REGISTRY[nodeSummary.type].buildInlets(nodeSummary.args)
    const testNodeOutlets = DEFAULT_REGISTRY[nodeSummary.type].buildOutlets(nodeSummary.args)
    ...
    Object.entries(testNodeOutlets).forEach(([outletId]) => {
        testNodeSinks[outletId] = [{nodeId: 'recorderNode', portletId: outletId}]
        recorderNodeSources[outletId] = [{nodeId: 'testNode', portletId: outletId}]
    })
    ...
    const nodeImplementations: NodeImplementations = {
        ...
        'testing-recorder': {
            ...
            setup: (_, {state}) => Object.keys(recorderNode.inlets).map(inletId => `
            ...
            loop: (_, {state, ins}) => Object.keys(recorderNode.inlets).map(inletId => `
                ${state('mem' + inletId)} = ${ins(inletId)}.slice ? ${ins(inletId)}.slice(0) : ${ins(inletId)}
            `).join('\n'),
        }
    }
    ...
    inputFrames.map((inputFrame) => {
        ...
        Object.keys(recorderNode.inlets).forEach((inletId) => {
            outputFrame[inletId] = getEngineNodeState(dspEngine, recorderNode.id, 'mem' + inletId)
        })
